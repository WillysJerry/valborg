...
void scan2_thrd(distribution dist, int id, par_array* out, void* f, void* p, void* args, void* cmp) { 
	...
	for(int i = 0; i < id; i++) {
		...
		prev_sum = func(prev_sum, VAL(pi_sum));
	}

	// Apply function f on the combined results of the previous blocks and each element in this block	
	for(int i = base; i < base + size; i++) {
		out->a[G2L(*out, i)] = SOME( func(prev_sum, VAL(ELEM(*A, i))));
	}
}

par_array vb_scan(double (*f)(double x, double y), double v, const par_array a, int (*p)(int i, par_array x, void* cmp), void* cmp) {
	distribution dist;
	...
	execute_in_parallel(scan1_thrd, dist, work_arrays, (void*)f, (void*)p, (void*)&v, cmp);
	...
	execute_in_parallel(scan2_thrd, dist, work_arrays + 1, (void*)f, NULL, (void*)&v, NULL);
	free_distribution(dist);
